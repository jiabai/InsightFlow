...
重要默认值与注意：
- MySQL 连接通过环境变量配置：
  - DB_HOST（默认 192.168.31.233）
  - DB_USER（默认 root）
  - DB_PASSWORD（默认 123456）
  - DB_NAME（默认 insight_flow）
  - 如你的 MySQL 地址/账户/密码不同，请设置对应的环境变量
...
```bash
# 开发：
python -m uvicorn server.api_services.api_services_main:app --app-dir src --host 0.0.0.0 --port 8000

# 或直接运行入口模块（同样需要保证 PythonPath 包含 src）：
# PYTHONPATH=src python src/server/api_services/api_services_main.py  # macOS/Linux
# $env:PYTHONPATH="src"; python src/server/api_services/api_services_main.py  # Windows PowerShell
```
...
- MySQL 连接失败
  - 通过 DB_HOST/DB_USER/DB_PASSWORD/DB_NAME 环境变量覆盖默认连接参数
- Redis 连接失败
