...
param (
    ...
    [switch]$NoModifyPath,
    [Parameter(HelpMessage = "Proxy server URL (e.g., http://127.0.0.1:7890)")]
    [string]$ProxyUrl,
...
function Download($download_url, $platforms) {
  ...
  if ($ProxyUrl -or $env:UV_PROXY_URL) {
    $proxy_address = if ($ProxyUrl) { $ProxyUrl } else { $env:UV_PROXY_URL }
    Write-Verbose "Using proxy: $proxy_address"
    $proxy = New-Object System.Net.WebProxy($proxy_address)
    $wc.Proxy = $proxy
  }
  ...
  if (($null -ne $info["updater"]) -and $install_updater) {
    ...
    # Configure proxy for updater download if provided
    if ($ProxyUrl -or $env:UV_PROXY_URL) {
      $proxy_address = if ($ProxyUrl) { $ProxyUrl } else { $env:UV_PROXY_URL }
      $proxy = New-Object System.Net.WebProxy($proxy_address)
      $wc.Proxy = $proxy
