    ...
    return new Promise((resolve) => {
        ...
        setTimeout(() => {
            if (endpoint === 'generate-questions') {
                ...
                resolve({ questions: shuffled.slice(0, Math.floor(Math.random() * 3) + 5) });
                ...
                resolve({ answer: answers[closestQuestion] || "这个问题需要更深入的分析和研究。从不同的角度来看，可能有多种答案。首先，我们需要考虑...（此处省略详细分析）" });
            }
        }, 800); // 模拟0.8秒的网络延迟
    });
}
...
function generateQuestion(text) {
    return simulateAPICall('generate-questions', { text });
}
function generateAnswer(question) {
    return simulateAPICall('generate-answer', { question });
}

export { generateQuestion, generateAnswer };